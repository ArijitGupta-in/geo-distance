...
function validateMinutes(minutes: number): void {
    if (!Number.isFinite(minutes) || minutes < 0 || minutes >= 60) {
        throw new RangeError(
            `Minutes must be within 0–59, got ${minutes}`
        );
    }
}
...
function validateSeconds(seconds: number): void {
    if (!Number.isFinite(seconds) || seconds < 0 || seconds >= 60) {
        throw new RangeError(
            `Seconds must be within 0–59, got ${seconds}`
        );
    }
}
...
/**
 * Converts a {@link DMSCoordinate} to a decimal-degree coordinate.
 *
 ...
 * ```
 */
export function dmsToDecimal(dms: DMSCoordinate): {
    latitude: number;
    longitude: number;
} {
    return {
